...
# List all nodes in a specific Kubernetes cluster
data "kakaocloud_kubernetes_engine_cluster_nodes" "all" {
  cluster_name = "<your-cluster-name>"
}

# List nodes from a specific node pool within the cluster
data "kakaocloud_kubernetes_engine_cluster_nodes" "by_pool" {
  cluster_name   = "<your-cluster-name>"
...
output "cluster_nodes" {
  description = "List of nodes in the specified cluster"
  value = {
    count = length(data.kakaocloud_kubernetes_engine_cluster_nodes.all.nodes)
    ids   = data.kakaocloud_kubernetes_engine_cluster_nodes.all.nodes[*].id
    names = data.kakaocloud_kubernetes_engine_cluster_nodes.all.nodes[*].name
  }
}
```

...
~ If `autoscaling` is **enabled**, you **must not** set `request_node_count`.  
~ If `autoscaling` is **disabled**, you **must** set `request_node_count`.

## Example Usage

```terraform
# kakaocloud_kubernetes_engine_node_pool Terraform Resource Example

# Basic Usage (kakaocloud_kubernetes_engine_node_pool)
data "kakaocloud_instance_flavors" "vm_flavor_m2a_large" {
  filter = [
    {
      name  = "instance_type"
      value = "vm"
...
data "kakaocloud_kubernetes_engine_images" "vm_image_k8s_v1_31_ubuntu_2204" {
  filter = [
    {
      name  = "instance_type"
      value = "vm"
...
resource "kakaocloud_kubernetes_engine_node_pool" "example" {
  name         = "example-node-pool"
  cluster_name = kakaocloud_kubernetes_engine_cluster.example.name

  image_id  = data.kakaocloud_kubernetes_engine_images.vm_image_k8s_v1_31_ubuntu_2204.images[0].id
  flavor_id = data.kakaocloud_instance_flavors.vm_flavor_m2a_large.instance_flavors[0].id
  ...
  ssh_key_name = kakaocloud_keypair.example.name
}

# Usage with Security Group (kakaocloud_kubernetes_engine_node_pool)
resource "kakaocloud_kubernetes_engine_node_pool" "example_sg" {
  name         = "example-node-pool-sg"
  cluster_name = kakaocloud_kubernetes_engine_cluster.example.name
  ...
  request_security_groups = [kakaocloud_security_group.example.id]
}
```

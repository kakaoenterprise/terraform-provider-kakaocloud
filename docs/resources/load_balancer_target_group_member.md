...
```terraform
# kakaocloud_load_balancer_target_group_member Terraform Resource Example

# Basic Usage (kakaocloud_load_balancer_target_group_member)
resource "kakaocloud_load_balancer_target_group_member" "example" {
  name            = "example-member"
  target_group_id = kakaocloud_load_balancer_target_group.example.id
  address         = kakaocloud_network_interface.example.private_ip
  protocol_port   = 80

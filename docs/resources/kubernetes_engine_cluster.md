...
```terraform
# kakaocloud_kubernetes_engine_cluster Terraform Resource Example

# Basic Usage (kakaocloud_kubernetes_engine_cluster)
resource "kakaocloud_kubernetes_engine_cluster" "example" {
  ...
  vpc_info = {
    id = kakaocloud_vpc.example.id
    subnets = [
      {
        id = kakaocloud_subnet.example_a.id
      },
      {
        id = kakaocloud_subnet.example_b.id
      }
    ]
  }
...
- `is_allocate_fip` (Required, Boolean) Whether to allocate a public IP <br/> - `true`: Allocate a public IP <br/> - `false`: Do not allocate a public IP
...
- `version` (Required, Attributes) Kubernetes version of the cluster ( see [below for nested schema](#nestedatt--version))
- `vpc_info` (Required, Attributes) VPC information where the cluster belongs ( see [below for nested schema](#nestedatt--vpc_info))
...
- `cni` (Required, String) CNI (Container Network Interface) plugin in use <br/> - `cilium`: High-performance eBPF-based CNI with low latency and strong security policies  <br/> - `calico`: Widely used CNI with simple configuration and multiple networking modes
...
- `subnets` (Required, Attributes Set) List of subnet information ( see [below for nested schema](#nestedatt--vpc_info--subnets))

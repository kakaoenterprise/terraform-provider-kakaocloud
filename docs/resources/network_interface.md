...
```terraform
# kakaocloud_network_interface Terraform Resource Example

# Basic Usage (kakaocloud_network_interface)
resource "kakaocloud_network_interface" "example" {
  name      = "example-ni"
  subnet_id = kakaocloud_subnet.example.id

  security_groups = [
    {
      id = kakaocloud_security_group.example.id
    }
  ]
}
```

...
data "kakaocloud_images" "image_ubuntu_2404" {
  filter = [
    {
      name  = "instance_type"
      value = "vm"
...
resource "kakaocloud_instance" "example" {
  ...
  image_id  = data.kakaocloud_images.image_ubuntu_2404.images[0].id

  subnets = [
    {
      id                   = kakaocloud_subnet.example.id
      network_interface_id = kakaocloud_network_interface.example.id
    }
  ]
  ...
  key_name = kakaocloud_keypair.example.name
}
```
